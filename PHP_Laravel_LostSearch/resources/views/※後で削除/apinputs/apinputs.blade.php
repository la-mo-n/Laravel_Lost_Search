...
<a href="apinputs?mode=del"  class="btn-square-select">削除</a>
  ...
  @foreach ($apinputs as $apinput)
    <h4>{{$apinput->municipality_cd}}</h4>
    <p>{{$apinput->municipality_nm}}</p>
    <p>{{$apinput->created_at}}</p>
    <hr>
    
    

   <option value="{{$apinput->municipality_cd}}">{{$apinput->municipality_nm}}</option>
...
<!--修正モード-->
 <div class="Form">
   ...
   <div class="Form-Item">
       <p class="Form-Item-Label"><span class="Form-Item-Label-Required">市町村コード</span></p>
   ...
   <!--<script type="text/javascript" src="public/api/ajaxzip3.js"></script>-->
   
   <script src="{{ asset('/dist/js/ajaxzip3.js') }}" defer></script>
